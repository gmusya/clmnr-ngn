...
# Create database with proper types if not exists
if [ ! -f "$DB_FILE" ]; then
...
SELECT * REPLACE (make_date(1970, 1, 1) + INTERVAL (EventDate) DAY AS EventDate)
FROM read_parquet('$PARQUET_FILE', binary_as_string=True);
EOF
    echo "Data loaded."
    echo ""
fi
...
declare -a QUERIES=(
    ...
    "SELECT WatchID, toDateTime(EventTime) AS EventTime, URL, Title FROM hits WHERE URL LIKE '%google%' ORDER BY EventTime LIMIT 10"
    "SELECT SearchPhrase, toDateTime(EventTime) AS EventTime FROM hits WHERE SearchPhrase <> '' ORDER BY EventTime LIMIT 10"
    "SELECT SearchPhrase FROM hits WHERE SearchPhrase <> '' ORDER BY SearchPhrase LIMIT 10"
    "SELECT SearchPhrase, toDateTime(EventTime) AS EventTime FROM hits WHERE SearchPhrase <> '' ORDER BY EventTime, SearchPhrase LIMIT 10"
    ...
    "SELECT DATE_TRUNC('minute', toDateTime(EventTime)) AS M, COUNT(*) AS PageViews FROM hits WHERE CounterID = 62 AND EventDate >= '2013-07-14' AND EventDate <= '2013-07-15' AND IsRefresh = 0 AND DontCountHits = 0 GROUP BY DATE_TRUNC('minute', toDateTime(EventTime)) ORDER BY M LIMIT 10 OFFSET 1000"
)
